...
Spending is bounded: paid sources stop being selected once the day's estimated cost reaches `WEB_RESEARCH_DAILY_BUDGET_USD` (default 1). Both limits hold when several processes share the state file: a call is booked and checked in one atomic statement before it goes out, and a paid call is refused rather than sent when the ledger cannot be written. Search pages served from a cursor or from the cache cost nothing. See [.env.example](.env.example) for every setting.
