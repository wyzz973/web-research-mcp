...
| `unsafe_url` | 指向私网、回环、云元数据地址；非 http(s)；HTTPS 降级跳转 | 拒绝，不重试 |
| `suspicious_url` | 查询串异常长或像是被塞了数据 | 默认拒绝，可配置 |
...
- 读取型工具也可能被用来外泄数据：注入内容可以诱导模型把秘密拼进 URL。对策是 `suspicious_url` 检查、所有出站地址写入本地审计日志、可配置的域名允许和拒绝列表。
- 严格模式（可选）：只允许抓取出现在我们自己输出里的地址，也就是搜索结果和已读页面里的链接。
