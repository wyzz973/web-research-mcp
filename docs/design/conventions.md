...
弱一些的模型经常把工具参数写错。能无歧义修复的就修复；有歧义的才报错。改变了请求含义的修复会在 `notes` 里说明，只是换了个写法的不说明。下表逐行对照代码实测过（2026-09-21，提交 `aca11eb`）。

| 输入 | 处理 | 写 note 吗 |
| --- | --- | --- |
| `sites` 或 `queries` 给了字符串（`web_search`） | 当作单元素数组 | 写 |
| `urls` 给了字符串（`web_fetch`） | 当作单元素数组 | **不写**。与 `web_search` 不一致，待统一 |
| 数字给了字符串 | 转成数字 | 不写：CLI 的参数永远是字符串，写了全是噪音 |
| URL 缺协议 | 补 `https://` | 写 |
| `sites` 里给了完整 URL | 取域名 | 写 |
| 查询里写了 `site:example.com` | 挪进 `sites` | 写 |
| 未知参数 | 忽略 | 写，列出参数名 |
| `max_tokens` 超过上限 | 取上限 | 写 |
| `query` 和 `queries` 同时给 | 合并去重 | 不写 |

容错的边界：`arguments` 本身必须是一个 JSON 对象。传 `null`、字符串或数组时，MCP SDK 在工具拿到输入之前就以协议错误（`-32602`）拒绝，这一层我们改不了，所以这种情况得不到带 `isError` 的工具结果。

让这些情况直接报错的「严格模式」（给测试环境用）属于 M1，**尚未实现**。
