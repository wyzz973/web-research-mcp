...
对抗审计（Opus）已完成四轮：设计、基础与外壳、搜索核心、抓取核心。这四轮的必须修项都已修复，并各补了一条不修就会失败的回归测试，结论记录在对应的提交说明里。第五轮（复核搜索侧的修复，并攻击 MCP、CLI、预算与成本护栏的真实行为）在 2026-09-21 进行中，**结论尚未返回**；`find` 的可见文字匹配、证据模式的选段改动（`visible.ts`、`modes.ts`、分块的递归下切、补上下文）**还没有经过审计**。
