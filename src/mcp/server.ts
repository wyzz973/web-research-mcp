...
export function createMcpServer(handlers: ToolHandlers, lifetime: AbortSignal): McpServer {
  const server = new McpServer({ name: 'web-research-mcp', version: '0.4.0' })
  const annotations = {
    readOnlyHint: true,
  ...
  server.registerTool(
    'webfetch',
    {
      ...
      description:
        'Fetch anonymous public HTML/text as readable Markdown or text. Supply url for a new fetch or cursor for an immutable saved snapshot, never both. max_chars counts Unicode code points. A nonempty next_cursor reads the next page. A search snapshot_cursor opens the full cited text; next_evidence_cursor opens a non-contiguous evidence view with more related paragraphs. The response view identifies document versus evidence. Omitted format inherits a cursor format.',
