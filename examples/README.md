...
当前业务输出为 0.2-draft。websearch.sites.input.json 演示域范围与证据选项；websearch.evidence.output.json 和 evidence-partial 例子展示成功及越界跳转缺证据。webfetch.evidence-snapshot.output.json 保存对应构造原文，用于独立校验 quote、hash 和 offsets。示例评分版本含 illustrative，不是本机已运行的评分器版本。
